...
wave_7_raw=paste(data_path,file_used, sep = "/") %>% 
  read.csv(., sep=",", dec = ",")
...
double<-inner_join(wave_7_raw,pilot_penr,by="penr")
double1<-inner_join(wave_7_raw,penrlist,by=c("penr"="PENR"))
test<-anti_join(double,double1,by="penr")
#no matter which list, the same 250 need to be kicked out
wave_7_raw<-anti_join(wave_7_raw,double, by="penr")
...
double<-inner_join(wave_7_raw,wave1_penr,by="penr")
wave_7_raw<-anti_join(wave_7_raw,double, by="penr")
#35 from first wave
...
double<-inner_join(wave_7_raw,wave2_penr,by="penr")
wave_7_raw<-anti_join(wave_7_raw,double, by="penr")
#1 from 2nd wave
...
double<-inner_join(wave_7_raw,wave3_penr,by="penr")
#0 from 3rd wave
...
double<-inner_join(wave_7_raw,wave4_penr,by="penr")
wave_7_raw<-anti_join(wave_7_raw,double, by="penr")
#1 from 4th wave

file_used = c("wave_5_assigned.xlsx")
wave5_penr = paste("B:/Randomization/2023-03/",file_used, sep = "/") %>% 
  read_excel()%>%select(penr)

double<-inner_join(wave_7_raw,wave5_penr,by="penr")
#none from 5th wave

file_used = c("wave_6_assigned.xlsx")
wave6_penr = paste("B:/Randomization/2023-05/",file_used, sep = "/") %>% 
  read_excel()%>%select(penr)

double<-inner_join(wave_7_raw,wave6_penr,by="penr")
#none from 6th wave

rm(double, test, double1, pilot_penr,penrlist,wave1_penr,wave2_penr,wave3_penr,wave4_penr,wave5_penr,wave6_penr)
...
#Korrekturen
wave_7$education[wave_7$ausb=="XX"]<-NA 
wave_7$nationality_AUT[wave_7$nation=="X"]<-NA 

wave_7=wave_7%>%select(penr,
                nationality_AUT, 
                male,
                agegr,
                region,
...
temp<-left_join(wave_7_raw,isco,by=c("beruf"),copy=TRUE)
temp$penr = as.integer(temp$penr)
temp<-temp%>%select(penr,beruf,ISCO08_1)

wave_7<-left_join(wave_7,temp,by="penr")
summary(wave_7$ISCO08_1)
