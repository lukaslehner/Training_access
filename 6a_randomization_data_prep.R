...
wave_6_raw=paste(data_path,file_used, sep = "/") %>% 
  read.csv(., sep=",", dec = ",")
...
double<-inner_join(wave_6_raw,pilot_penr,by="penr")
double1<-inner_join(wave_6_raw,penrlist,by=c("penr"="PENR"))
test<-anti_join(double,double1,by="penr")
#no matter which list, the same 286 need to be kicked out
wave_6_raw<-anti_join(wave_6_raw,double, by="penr")
...
double<-inner_join(wave_6_raw,wave1_penr,by="penr")
wave_6_raw<-anti_join(wave_6_raw,double, by="penr")
#15 from first wave
...
double<-inner_join(wave_6_raw,wave2_penr,by="penr")
wave_6_raw<-anti_join(wave_6_raw,double, by="penr")
#1 from 2nd wave
...
double<-inner_join(wave_6_raw,wave3_penr,by="penr")
wave_6_raw<-anti_join(wave_6_raw,double, by="penr")
#1 from 3rd wave
...
double<-inner_join(wave_6_raw,wave4_penr,by="penr")
#noone from 4th wave
...
double<-inner_join(wave_6_raw,wave5_penr,by="penr")
#noone from 5th wave

rm(double, test, double1, pilot_penr,penrlist,wave1_penr,wave2_penr,wave3_penr,wave4_penr,wave5_penr)
...
#Korrekturen
wave_6$education[wave_6$ausb=="XX"]<-NA 
wave_6$nationality_AUT[wave_6$nation=="X"]<-NA 

wave_6=wave_6%>%select(penr,
                nationality_AUT, 
                male,
                agegr,
                region,
...
temp<-left_join(wave_6_raw,isco,by=c("beruf"),copy=TRUE)
temp$penr = as.integer(temp$penr)
temp<-temp%>%select(penr,beruf,ISCO08_1)

wave_6<-left_join(wave_6,temp,by="penr")
summary(wave_6$ISCO08_1)
