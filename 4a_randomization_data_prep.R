...
data_path = "B:/Randomization/2023-01"

#load data
file_used=c("Datenlieferung_1-3.csv")
wave_4_raw=paste(data_path,file_used, sep = "/") %>% 
  read.csv(., sep=";" , dec = ",")
...
double<-inner_join(wave_4_raw,pilot_penr,by="penr")
double1<-inner_join(wave_4_raw,penrlist,by=c("penr"="PENR"))
test<-anti_join(double,double1,by="penr")
#no matter which list, the same 214 need to be kicked out
wave_4_raw<-anti_join(wave_4_raw,double, by="penr")
...
double<-inner_join(wave_4_raw,wave1_penr,by="penr")
wave_4_raw<-anti_join(wave_4_raw,double, by="penr")
#1 from first wave
...
double<-inner_join(wave_4_raw,wave2_penr,by="penr")
#noone from 2nd wave
...
double<-inner_join(wave_4_raw,wave3_penr,by="penr")
#noone from 3rd wave

rm(double, test, double1, pilot_penr,penrlist,wave1_penr,wave2_penr,wave3_penr)
...
#Korrekturen
wave_4$education[wave_4$ausb=="XX"]<-NA 
wave_4$nationality_AUT[wave_4$nation=="X"]<-NA 

wave_4=wave_4%>%select(penr,
                nationality_AUT, 
                male,
                agegr,
                region,
...
temp<-left_join(wave_4_raw,isco,by=c("beruf"),copy=TRUE)
temp$penr = as.integer(temp$penr)
temp<-temp%>%select(penr,beruf,ISCO08_1)

wave_4<-left_join(wave_4,temp,by="penr")
summary(wave_4$ISCO08_1)
